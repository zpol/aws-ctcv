...
if [ $# -lt 1 ]; then
    ...
    echo "-------------------------------------------"
    echo "Options: "
    echo " -f <filename>"	Specify an input file to analyze
    echo " -r "			Show all AWS regions
    echo ""
    exit 1
fi
...
# TODO 
# Add a coparison for all the aws regions we have in CT 
####
#REGIONS_ARRAY=("ap-south-1" "eu-north-1" "eu-west-3" "eu-west-2" "eu-west-1" "ap-northeast-3" "ap-northeast-2" "ap-northeast-1" "ca-central-1" "sa-east-1" "ap-southeast-1" "ap-southeast-2" "ap-southeast-3" "eu-central-1" "us-east-1" "us-east-2" "us-west-2")
